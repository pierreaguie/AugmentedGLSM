...
def get_levenshtein_distance(a, b):
    a, b = a.squeeze(2), b.squeeze(2)
    N1, S1 = a.size()
    N2, S2 = b.size()
    distance_matrix = np.zeros((N1, N2))
    for i in range(N1):
        a_i = torch.unique_consecutive(a[i])
        for j in range(N2):
            b_j = torch.unique_consecutive(b[j])
            norm = max(len(a_i), len(b_j))
            distance_matrix[i, j] = levenshtein_distance(a_i, b_j)/norm
...
def get_distance_group_dtw(a1, a2, size1, size2,
    ...
    if distance_function.__name__ == 'get_levenshtein_distance':
        distance_mat = torch.tensor(distance_function(a1, a2), dtype=torch.float)
        return distance_mat
    else:
